...
describe Spree::Issue do
  it "should be part of a product" do
    issue = build(:issue)
    issue.should respond_to(:product)
  end
  ...
  it "should be related to a product which is the product" do
    issue = create(:issue)
    issue.product.should be_an_instance_of Spree::Product
  end

  it "should not be valid if no product issue and no name is specified" do
    issue = build(:issue, name: "", product_issue: nil)
    issue.should_not be_valid
  end

  it "should have a name like the name of the product issue" do
    issue = create(:issue, product_issue: create(:base_product))
    issue.name.should equal(issue.product_issue.name)
  end

  it "should have the name attribute if no product issue is present" do
  ...
  it "should create a shipped issue when shipping issue" do
    ...
    issue = create(:issue, product: subscription.product)
